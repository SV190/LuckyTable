...
const toggleBlock = async (user) => {
  ...
  try {
    const token = getToken()
    console.log(`Блокировка пользователя ${user.id}, текущий статус: ${user.is_blocked}`)
    ...
    if (!response.ok) {
      ...
      console.error(`Ошибка блокировки: ${response.status} ${response.statusText}`, errorText)
      throw new Error(`Ошибка блокировки: ${response.status} ${response.statusText}`)
    }
    ...
    alert(`Пользователь ${user.username} успешно ${!user.is_blocked ? 'заблокирован' : 'разблокирован'}`)
  } catch (e) {
