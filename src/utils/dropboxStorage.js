...
export class DropboxStorageService {
  ...
  async downloadFile(filePath) {
    try {
      ...
      try {
        ...
        // Если это JSON с полем data и isBase64=true, это бинарный файл
        if (data.data && data.isBase64) {
          // Для Excel файлов возвращаем структуру с данными в base64
          if (filePath.toLowerCase().endsWith('.xlsx') || filePath.toLowerCase().endsWith('.xls')) {
            ...
            // Для других бинарных файлов преобразуем в ArrayBuffer
            // Используем безопасный метод декодирования base64
            const binaryString = this.safeAtob(data.data);
        ...
        // Если это обычный JSON, возвращаем его как есть
        return data;
      } catch (e) {
        // Если это не JSON, возвращаем текст как есть
        return text;
      }
    } catch (error) {
  ...
  // Безопасный метод декодирования base64, который работает с Unicode
  safeAtob(base64) {
    try {
      // Стандартный метод atob
      return atob(base64);
    } catch (e) {
      // Если стандартный метод не работает из-за Unicode символов,
      // используем более безопасный метод с поддержкой Unicode
      try {
        // Создаем Uint8Array из base64 строки
        ...
        console.error('Ошибка при декодировании base64:', e2);
        // Если и этот метод не сработал, возвращаем пустую строку
        return '';
      }
    }
  }
