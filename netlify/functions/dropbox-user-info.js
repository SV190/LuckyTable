...
exports.handler = async (event) => {
  try {
    ...
    // Получаем email и id
    const accountRes = await fetch('https://api.dropbox.com/2/users/get_current_account', {
      method: 'POST',
    ...
    const spaceRes = await fetch('https://api.dropbox.com/2/users/get_space_usage', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${accessToken}` }
    });
