...
exports.handler = async function(event, context) {
  ...
  if (event.httpMethod === 'GET') {
    try {
      ...
      // Функция для получения файлов с поддержкой пагинации
      async function listFiles(path, cursor = null) {
        let response;
        
        if (cursor) {
          response = await dbx.filesListFolderContinue({ cursor });
        } else {
          response = await dbx.filesListFolder({ path, recursive, limit: 100 });
        }
        
        // Добавляем файлы из текущего ответа
        ...
        if (response.result.has_more) {
          await listFiles(path, response.result.cursor);
        }
      }
      
      // Запускаем получение файлов
      await listFiles(folderPath);
      
      console.log(`Получено ${allFiles.length} файлов и папок из Dropbox`);
      ...
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ error: 'Ошибка получения файлов: ' + error.message })
      };
    }
  }
